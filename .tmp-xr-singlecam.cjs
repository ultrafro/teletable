...
(async()=>{
 const out={room:null,startedCameraIndex:null,hostErr:null,clientVideos:0,xrLogs:[],multiLogs:[],errors:[]};
 ...
 client.on('console',m=>{const t=m.text(); if(t.includes('[XRVideo]')) out.xrLogs.push(t); if(t.includes('[MultiCam Client]')) out.multiLogs.push(t);});
 try{
  const ts=Date.now();
  await signUpAndGoHome(host,`hostxrone${ts}@example.com`,'CodexPass123!');
  await host.getByRole('button',{name:/Create New Room|Creating Room/}).click();
  await host.waitForURL('**/host/**',{timeout:60000});
  out.room=host.url().split('/').pop();
  ...
  for(let i=0;i<n;i++){
    ...
    if(isChecked){ out.startedCameraIndex=i; break; }
  }
  const body=await host.locator('body').innerText();
  out.hostErr=(body.match(/Camera Error:[^\n]*/)||[])[0]||null;
  if(out.startedCameraIndex===null) throw new Error('No camera could be started in this environment');

  await signUpAndGoHome(client,`clientxrone${ts}@example.com`,'CodexPass123!');
  await client.goto(`http://localhost:3000/room/${out.room}`,{waitUntil:'domcontentloaded',timeout:60000});
  ...
  out.clientVideos=await client.locator('video').count();
