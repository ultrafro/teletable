...
(async()=>{
 ...
 host.on('console',m=>{const t=m.text(); if(/Adding camera stream|Removing camera stream|Answered initial call|additional camera|Calling client|incoming call|Switching stream|MultiCam/.test(t)) console.log('HOST',t)});
 client.on('console',m=>{const t=m.text(); if(/MultiCam|Camera stream changed/.test(t)) console.log('CLIENT',t)});
 const ts=Date.now();
 await signUpAndGoHome(host,`hostdbg${ts}@example.com`,'CodexPass123!');
 ...
 console.log('rows', await host.locator('text=Broadcasting').count());
 await signUpAndGoHome(client,`clientdbg${ts}@example.com`,'CodexPass123!');
