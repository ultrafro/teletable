...
export function detectionToHandDetection(
  ...
  console.log(currentHands.left.detected, currentHands.right.detected);
}
...
function extractTransform(
  ...
  const baseVector = new Vector3(base.x, base.y, base.z);
  const knuckleVector = new Vector3(knuckle.x, knuckle.y, knuckle.z);
  const pinkyVector = new Vector3(pinky.x, pinky.y, pinky.z);

  const centerVector = baseVector
    .clone()
    .add(knuckleVector)
    .add(pinkyVector)
    .divideScalar(3);
  ...
  const outVector = baseToPinkyVector
    .clone()
  ...
  const basisMat = new Matrix4().makeBasis(rightVector, upVector, outVector);

  const quaternion = new Quaternion();
  quaternion.setFromRotationMatrix(basisMat);

  const indexVector = new Vector3(index.x, index.y, index.z);
